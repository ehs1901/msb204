...
Dette avnsittet kommer lenger nede i arbeidskravet da vi hadde problemer med rekkefølgen på R-kodene.
...
-   **Subsidier for elektriske biler**: Støtte til innkjøp av elektriske kjøretøy eller tilskudd til hjemmeladere kan redusere den økonomiske terskelen for å velge miljøvennlige kjøretøy.
    ...
    <https://www.nettavisen.no/billig/str%C3%B8m/fordeler-og-ulemper-ved-elbiler/>
...
-   **Tilrettelegging for sykkel og kollektivtransport**: Ved å forbedre og subsidere kollektivtransport, sykkelveier og fotgjengertilgang kan myndighetene redusere avhengigheten av privatbiler, spesielt i byområder.
    ...
    <https://www.regjeringen.no/no/aktuelt/det-skal-bli-bedre-og-enklere-a-lade-bilen/id2950345/>

    <https://www.regjeringen.no/contentassets/75a2e25ed8af4eb5ae32b63c1b3d8353/handlingsplan-for-kollektivtransport.pdf>
...
-   **Forbud mot fossilbiler**: Etter hvert som teknologi og infrastruktur forbedres, kan myndighetene vurdere å sette en tidsfrist for forbud mot salg av nye fossilbiler, slik som Norge har satt som mål for 2025.
    ...
    <https://www.tv2.no/broom/forbud-mot-bensin-og-dieselbiler-kan-bli-utsatt/15554926/>
...
-   **Fordeler som gratis parkering og tilgang til kollektivfelt**: Myndighetene kan tilby insentiver som tilgang til kollektivfelt, billigere eller gratis parkering for elbiler og andre miljøvennlige alternativer, noe som gjør dem mer attraktive i hverdagen.
    ...
    <https://www.regjeringen.no/contentassets/65662b6bb87641d186edf4493fba1e77/2022-06-16-bruksfordeler-elbil-nett.pdf>
...
Denne tilnærmingen muliggjør en dypere analyse av hvordan ulike demografiske og logistiske faktorer påvirker valg av transportmiddel @zotero-204
...
=======
<<<<<<< HEAD
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
...
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
trafikk_data \<- read_excel("\~/Desktop/trafikk data/trafikk_data.xlsx", sheet = "in")

# Filtrer data for utvalgte stasjoner

stations_of_interest \<- c("Toskatjørn", "Haukås", "Grinde nord", "Karmøytunnelen Hellevik", "Karmøytunnelen Håvik") filtered_data \<- trafikk_data\[trafikk_data\$Navn %in% stations_of_interest, \]
<<<<<<< HEAD
...
stations_of_interest <- c("Toskatjørn", "Haukås", "Grinde nord", "Karmøytunnelen Hellevik", "Karmøytunnelen Håvik") 

filtered_data <- trafikk_data[trafikk_data$Navn %in% stations_of_interest,]
<<<<<<< HEAD

=======
>>>>>>> 007be02a6ea50591be655a5dc3bed8680a3170a5
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
...
s.na(filtered_data\$Trafikkmengde), \]

# Grupper data etter 'Navn' og 'År', og beregn gjennomsnittlig trafikkmengde per år

traffic_by_year_station \<- filtered_data %\>% group_by(Navn, År) %\>% summarise(AADT = mean(Trafikkmengde, na.rm = TRUE))
<<<<<<< HEAD
...
# Plot trafikkutviklingen med ggplot2

ggplot(traffic_by_year_station, aes(x = År, y = AADT, color = Navn, group = Navn)) + geom_line() + geom_point() + labs(title = "Årsdøgntrafikk, utvikling for utvalgte stasjoner", x = "År", y = "Gjennomsnittlig døgntrafikk") + theme_minimal() + scale_color_discrete(name = "Målestasjon") + theme(legend.position = "bottom")
<<<<<<< HEAD
...
# Utviklingen av disse utvalgte stasjonene mellom år 2010 og 2023 gir ett bilde på hvordan særlig Toskakjønn påvirkes av endringen som følger av åpning av karmøytunnelen i 2013. Utviklingen av grafen til Toskakjønn gir en betydelig redusert gjennomsnittlig andel døgntrafikk fom. år 2013, samme år som karmøytunnelen ble åpnet og som grafen viser tar en del av trafikken parallelt som Toskakjønn reduseres.

# Utviklingen generelt av disse kurvene er gjevnt stigende over de 13årene som blir målt, med unntak av en nedgang i 2019 - 2020, som kan forklares med covid-19 utbruddet og mange ble sittende på hjemmekontor. Denne utviklingen sees særlig i Grinde, en målestasjon fra ett mindre område og med mindre gjennomgående trafikk, men ett område som det antas har mange pendlere inn til Haugesund, Tysvær og Karmøy.
...
trafikk_data \<- read_excel("\~/Desktop/trafikk data/trafikk_data_2.xlsx", sheet = "in")
<<<<<<< HEAD
...
print(colnames(trafikk_data))

# Filtrer data for å beholde kun felt 1 og 2
...
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
trafikk_data \<- trafikk_data %\>% filter(Felt %in% c(1, 2))

# Kombiner dato og tidspunkt til en kolonne

trafikk_data \<- trafikk_data %\>% mutate(Dato = as.Date(Dato), Fra_tidspunkt = format(as.POSIXct(`Fra tidspunkt`, format="%Y-%m-%d %H:%M:%S", tz = "UTC"), "%H:%M:%S"), Datetime = ymd_hms(paste(Dato, Fra_tidspunkt))) %\>% mutate(Hour = hour(Datetime))

# kolonnenavn for kjøretøysammensetning

names(trafikk_data)\[names(trafikk_data) == "\< 5,6m"\] \<- "mindre_5_6m" names(trafikk_data)\[names(trafikk_data) == "\>= 5,6m"\] \<- "mer_5_6m" names(trafikk_data)\[names(trafikk_data) == "5,6m - 7,6m"\] \<- "mellom_5_6_og_7_6" names(trafikk_data)\[names(trafikk_data) == "7,6m - 12,5m"\] \<- "mellom_7_6_og_12_5" names(trafikk_data)\[names(trafikk_data) == "12,5m - 16,0m"\] \<- "mellom_12_5_og_16" names(trafikk_data)\[names(trafikk_data) == "\>= 16,0m"\] \<- "mer_16" names(trafikk_data)\[names(trafikk_data) == "16,0m - 24,0m"\] \<- "mellom_16_og_24" names(trafikk_data)\[names(trafikk_data) == "\>= 24,0m"\] \<- "mer_24"
<<<<<<< HEAD
...
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
timelig_trafikk \<- trafikk_data %\>% group_by(Hour) %\>% summarise(Trafikkmengde = sum(Trafikkmengde, na.rm = TRUE))

# Analyserer sammensetning av kjøretøy

kjoretoy_sammensetning \<- trafikk_data %\>% group_by(Hour) %\>% summarise( Passasjerbiler = sum(mindre_5_6m + mellom_5_6_og_7_6, na.rm = TRUE),\
Lastebiler = sum(mellom_7_6_og_12_5 + mellom_12_5_og_16 + mer_16 + mellom_16_og_24 + mer_24, na.rm = TRUE), Totalt = sum(Trafikkmengde, na.rm = TRUE) ) %\>% mutate( Passasjerandel = Passasjerbiler / Totalt, Lastebilandel = Lastebiler / Totalt )
<<<<<<< HEAD
...
# Plott trafikk per time

ggplot(timelig_trafikk, aes(x = Hour, y = Trafikkmengde)) + geom_line() + labs(title = "Gjennomsnittlig trafikk per time", x = "Tid på dagen", y = "Trafikkmengde") + scale_y_continuous(labels = NULL) + theme_minimal()

# Plott andel av kjøretøy per time

<<<<<<< HEAD
=======
ggplot(kjoretoy_sammensetning, aes(x = Hour)) + geom_line(aes(y = Passasjerandel, color = "Personbiler")) + geom_line(aes(y = Lastebilandel, color = "Lastebiler")) + labs(title = "Gjennomsnittlig kjøretøysammensetning per time", x = "Tid på dagen", y = "Andel trafikk i %") + scale_color_manual(values = c("blue", "red")) + theme_minimal()
=======
<<<<<<< HEAD
>>>>>>> 32ebc10e1b2d5263f82630abfd75f7bb9c397b98
>>>>>>> 1e1f65824aebc270541c3b3cacb7b022eb6dedcc
...
# Grafen for gjennomsnittlig trafikk per time viser økende trafikk som starter kl 05.00 - 08.00, deretter hvor trafikken reduseres lett, før den igjen øker fra kl. 09.00 og stiger gjevnt frem mot sitt toppppunkt kl 15.00, før den totale trafikken faller merkant frem mot kl 22.00. Denne tendensen tyder på at det er mye pendling gjennom Karmsundsgaten med sine topp punkt i typisk start/slutt arbeidstid (kl 07-08 og kl. 15-16). Det observeres likevel en jevn økning inn mot kl. 15, som antas å være gjennomgående trafikk, varetransport eller annet, som også er av en betydelig andel.

# Grafen for gjennomsnittlig kjøretøysammensetning per time viser at det er gjennomgående mest personbiler med en andel på over 90 %. Grafen viser likevel en økning i antall lastebiler på nattestid mellom kl. 02.00 og kl. 05.00, før den igjen får en liten stigning til frem mot kl. 09.00, og avtar deretter resten av døgnet.
<<<<<<< HEAD
...
library(readxl) library(dplyr)\
library(ggplot2) library(lubridate) ======= Grafen for gjennomsnittlig kjøretøysammensetning per time viser at det er gjennomgående mest personbiler med en andel på over 90 %.
...
daglig_trafikk \<- trafikk_data %\>% group_by(Dato) %\>% summarise(Total_trafikk = sum(Trafikkmengde, na.rm = TRUE))
=======
...
# Plott total trafikk per dag

ggplot(daglig_trafikk, aes(x = Dato, y = Total_trafikk)) + geom_line(color = "darkgreen") + labs(title = "Total trafikk per dag", x = "Dato", y = "Total trafikk") + theme_minimal()
...
Det sees noe variasjon i total trafikkmengde per dag over perioden som er analysert som kan forklares med ferieavvikling, særlig i perioden d.d.
