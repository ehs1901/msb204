...
```{r}
#| echo: false
# 1. Forberedelse av data: Velg relevante variabler fra transport_splitt
inntekt_transport <- transport_splitt %>%
  # Velg relevante kolonner
  select(Inntektsgruppe, Hvordan_kommer_du_deg_til_jobb) %>%
  # Fjern rader med NA-verdier i både Inntektsgruppe og Hvordan_kommer_du_deg_til_jobb
  filter(!is.na(Inntektsgruppe) & !is.na(Hvordan_kommer_du_deg_til_jobb)) %>%
  # Omkategorisere inntektsgruppene i henhold til tidligere beskrivelse
  ...
  # Fjern rader med "Ønsker ikke å oppgi", "Foretrekker å ikke si" og NA-verdier
  filter(!Inntektsgruppe %in% c("Ønsker ikke å oppgi", "Foretrekker å ikke si") & !is.na(Inntektsgruppe))

# 2. Visualisering: Fordeling av transportmiddel etter inntektsgruppe
ggplot(inntekt_transport, aes(x = Inntektsgruppe, fill = Hvordan_kommer_du_deg_til_jobb)) +
  geom_bar(position = "fill") +
  ylab("Prosent") +
  xlab("Inntektsnivå") +
...
```{r}
transport_splitt <- multinom(Hvordan_kommer_du_deg_til_jobb ~ Tid_brukt_til_jobb + Inntektsgruppe + Alder + Kjønn + Utdanning, data = clean_data)
```

```{r}
summary(transport_splitt)
```
...
stargazer(transport_splitt, type = "text")
```
